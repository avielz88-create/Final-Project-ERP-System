...
**האפליקציה החיה:** [erp-buddy-il.lovable.app](https://erp-buddy-il.lovable.app)

## בקצרה

- **10 workflows** ב-n8n — אימות חשבוניות ומע"מ, הפקת מסמכים, קליטת לידים, מיילי מכירות, וחיבור לאפליקציה.
- **3 סוכני AI** בטלגרם/באפליקציה: שירות לקוחות, מכירות (יזום + מעקב תשובות), וסוכן מנהל (אנליטיקה, מוגבל לבעלים בלבד).
- **RAG אמיתי** — הסוכנים מבוססים על [12 מסמכי מדיניות](policy-source/) אמיתיים (החזרות, אחריות, משלוחים, תשלומים, חוקי מע"מ, guardrails) ועל [קטלוג של 34 מוצרים](products.csv) אמיתיים, לא דוגמאות מומצאות.
- **Airtable** כמסד הנתונים היחיד (4 טבלאות), **Lovable** כשכבת הממשק שמדברת עם n8n דרך webhook אחד.
- **נבדק מקצה לקצה** עם נתונים אמיתיים: מיילים אמיתיים שנשלחו, הודעות טלגרם אמיתיות, חשבונית שהופקה ועלתה לדרייב, וקבוצת באגים שנמצאו ותוקנו תוך כדי (מתועדים למטה).
...
- [מקורות התוכן ל-RAG](#מקורות-התוכן-ל-rag)
...
| WF7 | [מוצרים → מאגר וקטורי](n8n-workflows/WF7-products-vector-store.json) | ידני | קורא את קטלוג 34 המוצרים האמיתיים (ראו [`products.csv`](products.csv)) מ-Airtable ומוסיף אותם לאותו מאגר וקטורי | ✅ |
...
- [`products.csv`](products.csv) — קטלוג 34 המוצרים האמיתיים שיובאו ל-Airtable ונטענים ב-WF7.
